...
def main():
    ...
    fcst_rows, ranked_days, max_day = GLIP_Functions.get_row_dicts(current_day_future_gen_tab, next_day_future_gen_tab,
                                                                   second_half_of_current_day)

    # Process rows to copy latest day if fcst is missing days (like on weekends and holidays)
    final_fcst_rows = GLIP_Functions.ensure_six_days(fcst_rows, ranked_days, max_day)
    ...
    for template_row in output_template:
        ...
        for fcst_row in final_fcst_rows:
            # print(fcst_row)
